...
def pyrdown_impl(image):
    ...
    fltrd1 = cv2.filter2D(src=image, kernel=kern, mode = BORDER_REFLECT_101)
    fltrd2 = cv2.filter2D(src=fltrd1, kernel=np.transpose(kern), mode = BORDER_REFLECT_101)
    if len(shape) == 2:
        down = fltrd2[::2,::2]
    else: 
        down = fltrd2[::2,::2, :]
...
def pyrup_impl(image):
    ...
    fltrd1 = cv2.filter2D(src=mixed, kernel=kern, mode = BORDER_REFLECT_101)
    fltrd2 = cv2.filter2D(src=fltrd1, kernel=np.transpose(kern), mode = BORDER_REFLECT_101)
    return fltrd2
...
def preprocess_ncc_impl(image, ncc_size):
    ...
    #subtract result of the mean filter from the image
    #mean = scipy.ndimage.uniform_filter(image, size=ncc_size, mode='constant', cval=0.0)
